...
interface PushLockMainAppProps {
  initialTab?: ActiveTab | 'time';
}

export function PushLockMainApp({ initialTab = 'home' }: PushLockMainAppProps) {
  // Navigation
  const [activeTab, setActiveTab] = useState<ActiveTab | 'time'>(initialTab);
  ...
  useEffect(() => {
    ...
    async function loadNativeData() {
      ...
      // 4. Check for any pending lock trigger intent
      try {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] text-gray-900 flex flex-col font-sans selection:bg-emerald-200">
      {/* Web Navigation Header to return to Landing Page */}
      <div className="bg-slate-900 text-slate-300 border-b border-slate-800 text-xs py-1.5 px-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          >
            ...
            <span>Back to PushLock AI Home</span>
          </Link>
          <div className="flex items-center gap-3 text-[11px] text-slate-400">
            <Link
              href="/app"
              className={`hover:text-white transition-colors ${activeTab !== 'workout' ? 'text-emerald-400 font-bold' : ''}`}
            >
              Dashboard
            </Link>
            <span>•</span>
